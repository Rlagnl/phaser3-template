...
export default class playGame extends GameScene {
  gameState: number
  gameOver: boolean
  level: number
  ...
  public create($data): void {
    super.create($data)
    // 背景
    this.setBackgroundColor(0x444444)
    ...
    this.blockSize = CONSTS.DESIGN_WIDTH / gameOptions.blocksPerLine;
    ...
    this.emptySpace = CONSTS.DESIGN_HEIGHT - this.gameFieldHeight;

    // set bounds of the physics world
    this.physics.world.setBounds(0, this.emptySpace / 2, CONSTS.DESIGN_WIDTH, this.gameFieldHeight);
    ...
    let scorePanel = this.add.sprite(CONSTS.DESIGN_WIDTH / 2, 0, "panel");
    scorePanel.displayWidth = CONSTS.DESIGN_WIDTH;
    scorePanel.displayHeight = this.emptySpace / 2;
    scorePanel.setOrigin(0.5, 0);

    // bottom panel
    this.bottomPanel = this.add.sprite(CONSTS.DESIGN_WIDTH / 2, CONSTS.DESIGN_HEIGHT, "panel");
    this.bottomPanel.displayWidth = CONSTS.DESIGN_WIDTH;
    this.bottomPanel.displayHeight = this.emptySpace / 2;
    this.bottomPanel.setOrigin(0.5, 1);

    // determine actual ball size in pixels
    this.ballSize = CONSTS.DESIGN_WIDTH * gameOptions.ballSize;

    // add the first ball
    this.addBall(CONSTS.DESIGN_WIDTH / 2, CONSTS.DESIGN_HEIGHT - this.bottomPanel.displayHeight - this.ballSize / 2, false);
    // this.addBall(CONSTS.DESIGN_WIDTH / 2, 0, false);
  ...
  addBlockLine() {
    ...
    let boundRectangle = new Rectangle(0, this.emptySpace / 2, CONSTS.DESIGN_WIDTH, this.gameFieldHeight);
  ...
  getBallPosition() {
    ...
    let children = this.ballGroup.getChildren() as Ball[];
  ...
  adjustAim(e) {
    ...
    if (this.gameState == PLAYER_IS_AIMING) {
      ...
      const ratio = CONSTS.WINDOW_HEIGHT / CONSTS.DESIGN_HEIGHT
      const mouseX = e.x / ratio + cr.x
      const mouseY = e.y / ratio - cr.y
      const { x: ballX, y: ballY } = this.getBallPosition()

      // determine x and y distance between current and initial input
      let distY = ballY - mouseY;
      ...
      if (distY > 10) {
        ...
        this.direction = Phaser.Math.Angle.Between(ballX, ballY, mouseX, mouseY);
  ...
  handleBallVsExtra() {
    ...
    this.physics.world.overlap(this.ballGroup, this.extraBallGroup, (ball, extraBall) => {
      ...
      this.tweens.add({
        ...
        y: CONSTS.DESIGN_HEIGHT - this.bottomPanel.displayHeight - _extraBall.displayHeight / 2,
